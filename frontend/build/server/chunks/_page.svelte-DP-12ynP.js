					...
					children: ($$renderer) => {
						...
						if (each_array.length !== 0) {
							...
							for (let $$index = 0, $$length = each_array.length; $$index < $$length; $$index++) {
								...
								$$renderer.push(`<tr class="border-border border-b last:border-0"><td class="py-3 font-medium">${escape_html(a.school?.name ?? "-")}</td><td class="py-3">${escape_html(a.category?.name ?? "-")}</td><td class="text-muted-foreground py-3">${escape_html(a.windowStart?.split("T")[0] ?? "-")}</td><td class="text-muted-foreground py-3">${escape_html(a.windowEnd?.split("T")[0] ?? "-")}</td><td class="py-3">`);
								Badge($$renderer, {
									variant: a.active ? "default" : "secondary",
									children: ($$renderer) => {
										$$renderer.push(`<!---->${escape_html(a.active ? "aktif" : "pasif")}`);
									},
									$$slots: { default: true }
								});
								$$renderer.push(`<!----></td><td class="flex gap-3 py-3"><button type="button" class="text-xs text-blue-600 hover:underline">Edit</button> <form method="POST" action="?/delete"><input type="hidden" name="id"${attr("value", a.id)}/> <button type="submit" class="text-destructive text-xs hover:underline">Hapus</button></form></td></tr>`);
							}
						} else {
							$$renderer.push("<!--[!-->");
...
//# sourceMappingURL=_page.svelte-DP-12ynP.js.map
