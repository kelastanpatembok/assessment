import { $ as head, ae as ensure_array_like, a1 as escape_html, _ as derived } from './dev-Ye9HvMQi.js';
					...
					children: ($$renderer) => {
						...
						if (each_array_1.length !== 0) {
							...
							for (let $$index_1 = 0, $$length = each_array_1.length; $$index_1 < $$length; $$index_1++) {
								...
								$$renderer.push(`<tr class="border-border border-b last:border-0"><td class="py-3 font-medium">${escape_html(r.studentName)}</td><td class="text-muted-foreground py-3">${escape_html(r.schoolName ?? "-")}</td><td class="text-muted-foreground py-3">${escape_html(r.completedAt ?? r.createdAt ? new Date(r.completedAt ?? r.createdAt).toLocaleDateString("id-ID") : "-")}</td><td class="py-3">`);
								if (activeTab === "disc") {
									$$renderer.push("<!--[0-->");
									...
									$$renderer.push("<!--[1-->");
									$$renderer.push(`${escape_html(r.hollandCode ?? "R:" + (r.rscore ?? 0) + " I:" + (r.iscore ?? 0) + " A:" + (r.ascore ?? 0))}`);
								} else if (activeTab === "cfit") {
									$$renderer.push("<!--[2-->");
									$$renderer.push(`RS: ${escape_html(r.totalScore ?? "-")} IQ: ${escape_html(r.iqScore ?? "-")}`);
								} else if (activeTab === "ist") {
									$$renderer.push("<!--[3-->");
									$$renderer.push(`IQ: ${escape_html(r.iqScore ?? "-")} (${escape_html(r.iqCategory ?? "-")})`);
								} else {
									$$renderer.push("<!--[-1-->");
...
//# sourceMappingURL=_page.svelte-BZ0A0raf.js.map
