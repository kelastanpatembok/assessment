					...
					children: ($$renderer) => {
						...
						if (each_array.length !== 0) {
							...
							for (let i = 0, $$length = each_array.length; i < $$length; i++) {
								...
								$$renderer.push(`<tr class="border-border border-b last:border-0"><td class="py-3">${escape_html(i + 1)}</td><td class="py-3 font-medium">${escape_html(school.schoolName)}</td><td class="text-muted-foreground py-3">${escape_html(school.address ?? "-")}</td><td class="py-3"><form method="POST" action="?/delete"><input type="hidden" name="id"${attr("value", school.id)}/> <button type="submit" class="text-destructive text-xs hover:underline">Hapus</button></form></td></tr>`);
							}
						} else {
							$$renderer.push("<!--[!-->");
...
//# sourceMappingURL=_page.svelte-BAuHmyMc.js.map
