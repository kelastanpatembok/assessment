			...
			Card($$renderer, {
				children: ($$renderer) => {
					Card_header($$renderer, {
						children: ($$renderer) => {
							Card_title($$renderer, {
								class: "text-base",
								children: ($$renderer) => {
									$$renderer.push(`<!---->${escape_html(r().type1)} — ${escape_html(r().type1Name)}`);
								},
								$$slots: { default: true }
							});
						},
					...
					if (r().type1Desc) {
						...
						Card_content($$renderer, {
							children: ($$renderer) => {
								$$renderer.push(`<p class="text-muted-foreground text-sm leading-relaxed">${escape_html(r().type1Desc)}</p>`);
							},
							$$slots: { default: true }
						});
...
//# sourceMappingURL=_page.svelte-DCEqJK8J.js.map
