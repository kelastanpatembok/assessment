			...
			for (let i = 0, $$length = each_array.length; i < $$length; i++) {
				...
				Card($$renderer, {
					children: ($$renderer) => {
						...
						Card_content($$renderer, {
							children: ($$renderer) => {
								$$renderer.push(`<p class="text-muted-foreground mb-4 text-sm">Pilih satu pernyataan yang lebih tepat menggambarkan Anda:</p> <div class="flex flex-col gap-3"><label class="hover:bg-accent flex cursor-pointer items-start gap-3 rounded-lg border p-4 transition-colors"><input type="radio"${attr("name", `pair_${stringify(pair.pairNo)}`)} value="A" class="mt-0.5 size-4 shrink-0" required=""/> <span class="text-sm">${escape_html(pair.stmtA)}</span></label> <label class="hover:bg-accent flex cursor-pointer items-start gap-3 rounded-lg border p-4 transition-colors"><input type="radio"${attr("name", `pair_${stringify(pair.pairNo)}`)} value="B" class="mt-0.5 size-4 shrink-0" required=""/> <span class="text-sm">${escape_html(pair.stmtB)}</span></label></div>`);
							},
							$$slots: { default: true }
						});
						$$renderer.push(`<!---->`);
...
//# sourceMappingURL=_page.svelte-CvHZ1kSp.js.map
