				...
				for (let qi = 0, $$length = each_array_2.length; qi < $$length; qi++) {
					...
					Card($$renderer, {
						children: ($$renderer) => {
							...
							Card_content($$renderer, {
								children: ($$renderer) => {
									$$renderer.push(`<p class="mb-4 text-sm">${escape_html(q.question)}</p> `);
									if (optionEntries(q).length > 0) {
										...
										for (let $$index_1 = 0, $$length = each_array_3.length; $$index_1 < $$length; $$index_1++) {
											...
											$$renderer.push(`<label class="hover:bg-accent flex cursor-pointer items-center gap-2 rounded-lg border p-3 text-sm"><input type="radio"${attr("name", `cfit_${stringify(q.id)}`)}${attr("value", optKey)} class="size-4 shrink-0" required=""/> <span>${escape_html(optKey)}. ${escape_html(optVal)}</span></label>`);
										}
										$$renderer.push(`<!--]--></div>`);
									} else {
										$$renderer.push("<!--[-1-->");
										$$renderer.push(`<input type="text"${attr("name", `cfit_${stringify(q.id)}`)} placeholder="Jawaban..." class="border-input bg-background flex h-10 w-full max-w-xs rounded-lg border px-3 text-sm" required=""/>`);
									}
									$$renderer.push(`<!--]-->`);
								},
								$$slots: { default: true }
...
//# sourceMappingURL=_page.svelte-Bk4Ewzpn.js.map
