<script lang="ts">
  ...
  const navLinks = [
    { href: '/afiliator-dashboard', label: 'Dashboard' },
    { href: '/afiliator-fees', label: 'Komisi' },
  ];
</script>
...
<div class="bg-background flex min-h-screen">
  <aside class="bg-card border-border flex w-60 flex-col border-r">
    <div class="border-border border-b px-6 py-5">
      ...
      <p class="text-muted-foreground mt-1 text-xs">Afiliator</p>
    </div>
  ...
  <div class="flex flex-1 flex-col">
    <header class="bg-card border-border border-b px-6 py-4">
      <div class="flex items-center justify-between">
        <h1 class="font-semibold">Panel Afiliator</h1>
