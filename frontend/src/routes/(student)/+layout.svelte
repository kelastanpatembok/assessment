<script lang="ts">
  ...
  const navLinks = [
    ...
    { href: '/student-disc', label: 'Tes DISC' },
    { href: '/student-holland', label: 'Tes Holland' },
    { href: '/student-papi', label: 'Tes PAPI Kostick' },
    { href: '/student-cfit', label: 'Tes IQ CFIT' },
    { href: '/student-ist', label: 'Tes IQ IST' },
  ];
</script>
...
<div class="bg-background flex min-h-screen">
  <aside class="bg-card border-border flex w-60 flex-col border-r">
    <div class="border-border border-b px-6 py-5">
      ...
      <p class="text-muted-foreground mt-1 text-xs">Siswa</p>
    </div>
          ...
          <li>
            <a
              ...
              class="hover:bg-accent hover:text-accent-foreground flex items-center rounded-lg px-3 py-2 text-sm transition-colors
                {$page.url.pathname.startsWith(link.href) && link.href !== '/student-dashboard'
  ...
  <div class="flex flex-1 flex-col">
    <header class="bg-card border-border border-b px-6 py-4">
      <div class="flex items-center justify-between">
        <h1 class="font-semibold">Panel Siswa</h1>
