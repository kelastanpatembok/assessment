<script lang="ts">
  ...
  async function load() {
    ...
    error = null;
    try {
      console.log('Client fetching from:', `${BASE}/big5/questions`);
      console.log('BASE URL:', BASE);
      
      // Test fetch dengan credentials
      ...
      if (!res.ok) {
        ...
        throw new Error(`Gagal memuat pertanyaan: ${res.status} ${res.statusText}`);
      }
      
      questions = await res.json();
      console.log('Success! Got questions:', questions.length);
    } catch (e) {
